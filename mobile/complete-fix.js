...
// Find all JobDetailsScreen occurrences
const regex = /function JobDetailsScreen\([^)]*\)\s*\{[\s\S]*?\n\}/g;
const matches = content.match(regex);
console.log(`Found ${matches?.length || 0} JobDetailsScreen definitions`);

// Remove everything from the first JobDetailsScreen to the end of the file
// and append a clean version

// Find the start of the first JobDetailsScreen
const startIdx = content.indexOf('function JobDetailsScreen({ route, navigation })');
if (startIdx === -1) {
  console.log('Could not find JobDetailsScreen');
  process.exit(1);
}

// Find where the component ends (look for the next function or the end)
let endIdx = content.indexOf('function ', startIdx + 1);
if (endIdx === -1) {
  endIdx = content.length;
}

console.log(`Removing from ${startIdx} to ${endIdx}`);

// Keep everything before the JobDetailsScreen
const beforeContent = content.substring(0, startIdx);

// Create a clean JobDetailsScreen
const cleanJobDetails = `function JobDetailsScreen({ route, navigation }) {
...
// Write the new file
const newContent = beforeContent + cleanJobDetails + content.substring(endIdx);
fs.writeFileSync(filePath, newContent, 'utf8');
console.log('✅ JobDetailsScreen completely replaced!');
console.log('   The updateStatus function is now properly defined.');
